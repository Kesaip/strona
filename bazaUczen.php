        ...
        if (!preg_match("/^[a-zA-Z]+$/", $_POST['Nazwisko'])) {
            ...
            if (!preg_match("/^[a-zA-Z]+$/", $Imie)) {
                ...
                if ($_POST["haslo"] == $_POST["haslo2"]) {
                    $Zapytanie = "INSERT INTO uczniowie (Imie,Nazwisko,Email,Haslo) VALUES ('" . $Imie . "','" . $_POST["Nazwisko"] . "','" . $Email . "','" . $_POST["haslo"] . "')";
                    if ($conn->query($Zapytanie) === true) {
                        $Zapytanie2 = "SELECT uczenId,Email FROM uczniowie WHERE Email = ('" . $_POST["Email"] . "')";
                        $result = mysqli_query($conn, $Zapytanie2);
                        $row = mysqli_fetch_assoc($result);
                        $kl = "SELECT klasaId FROM klasy WHERE klasa = '" . $_POST['klasa'] . "'";
                        $klasa = mysqli_query($conn, $kl);
                        $row2 = mysqli_fetch_assoc($klasa);
                        $Zapytanie3 = "INSERT INTO przydzial (uczen,klasa1) VALUES ('" . $row["uczenId"] . "','" . $row2["klasaId"] . "')";
